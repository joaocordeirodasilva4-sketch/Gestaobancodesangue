...
import { DoacoesService } from '../../core/services/doacoes.service';
import { DoadoresService } from '../../core/services/doadores.service';
import { ToastService } from '../../core/services/toast.service';
import { Doacao, DoacaoCreate, TipoSanguineo, StatusDoacao } from '../../core/models';
...
@Component({
  ...
  template: `
    <div class="page">
      <div class="page-header"><div><h1>Doações</h1><p>{{ total() }} doações registradas</p></div><button class="btn-primary" (click)="openModal()">+ Registrar Doação</button></div>
      <div class="search-bar">
        ...
        <input [(ngModel)]="search" placeholder="Buscar por doador ou tipo sanguíneo..." (input)="onSearch()" />
      </div>
      <div class="table-wrap">
        @if (loading()) { <div class="table-loading">@for(i of [1,2,3]; track i){<div class="skel"></div>}</div>
        } @else {
          <table>
            <thead><tr><th>Doador</th><th>Tipo</th><th>Data</th><th>Volume</th><th>Status</th><th>Ações</th></tr></thead>
            <tbody>
              @for (d of doacoes(); track d.id) {
                <tr>
                  <td><p class="nome">{{ d.doador_nome ?? '—' }}</p></td>
                  <td><span class="badge-tipo" [class]="tipoCss(d.tipo_sanguineo)">{{ d.tipo_sanguineo }}</span></td>
                  <td>{{ d.data_doacao }}</td><td>{{ d.volume_ml }} ml</td>
                  <td><span class="badge-status" [class]="'st-'+d.status">{{ sLabel(d.status) }}</span></td>
                  <td class="actions">
                    <button class="icon-btn" (click)="openModal(d)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                    <button class="icon-btn danger" (click)="delete(d.id)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>
                  </td>
                </tr>
              } @empty { <tr><td colspan="6" class="empty">Nenhuma doação registrada.</td></tr> }
            </tbody>
          </table>
        }
      </div>
    ...
    @if (showModal()) {
      <div class="overlay" (click)="closeModal()">
        <div class="modal" (click)="$event.stopPropagation()">
          <div class="modal-header"><h2>{{ editId() ? 'Editar Doação' : 'Registrar Doação' }}</h2><button class="close" (click)="closeModal()">✕</button></div>
          <div class="form-group"><label>Doador *</label>
            <select [(ngModel)]="form.doador_id">
              <option value="">Selecione um doador</option>
              @for(d of doadores_lista(); track d.id){ <option [value]="d.id">{{ d.nome_completo }}</option> }
            </select>
          </div>
          <div class="form-row">
            <div class="form-group"><label>Tipo Sanguíneo</label>
              <select [(ngModel)]="form.tipo_sanguineo"><option value="">Auto</option>@for(t of tipos;track t){<option [value]="t">{{t}}</option>}</select>
            </div>
            <div class="form-group"><label>Data da Doação</label><input type="date" [(ngModel)]="form.data_doacao"/></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>Volume (ml)</label><input type="number" [(ngModel)]="form.volume_ml"/></div>
            <div class="form-group"><label>Status</label>
              <select [(ngModel)]="form.status"><option value="coletada">Coletada</option><option value="em_analise">Em Análise</option><option value="aprovada">Aprovada</option><option value="rejeitada">Rejeitada</option><option value="utilizada">Utilizada</option></select>
            </div>
          </div>
          <div class="form-group"><label>Observações</label><textarea [(ngModel)]="form.observacoes"></textarea></div>
          <div class="modal-footer">
            <button class="btn-cancel" (click)="closeModal()">Cancelar</button>
            <button class="btn-primary" (click)="save()" [disabled]="saving()">{{ saving() ? 'Salvando...' : (editId() ? 'Salvar' : 'Registrar') }}</button>
          </div>
        </div>
      </div>
    }
  ...
  styles: [`
    ...
    table{width:100%;border-collapse:collapse}thead tr{background:#f9fafb}th{text-align:left;font-size:12px;font-weight:600;color:#6b7280;padding:12px 16px;border-bottom:1px solid #e5e7eb;text-transform:uppercase;letter-spacing:.03em}td{padding:14px 16px;font-size:13px;color:#111;border-bottom:1px solid #f3f4f6;vertical-align:middle}tr:last-child td{border-bottom:none}.nome{font-weight:600}.empty{text-align:center;color:#9ca3af;padding:40px!important}
    ...
    .badge-status{display:inline-block;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600}.st-coletada{background:#dbeafe;color:#1d4ed8}.st-em_analise{background:#fef3c7;color:#d97706}.st-aprovada{background:#dcfce7;color:#16a34a}.st-rejeitada{background:#fde8e8;color:#c0392b}.st-utilizada{background:#f3f4f6;color:#6b7280}
...
export class DoacoesComponent implements OnInit {
  private svc        = inject(DoacoesService);
  private doaSvc     = inject(DoadoresService);
  private toast      = inject(ToastService);
  doacoes = signal<Doacao[]>([]); doadores_lista = signal<any[]>([]); total = signal(0); page = signal(1); pages = signal(1);
  loading = signal(true); saving = signal(false); showModal = signal(false); editId = signal<number|null>(null);
  search = ''; tipos = TIPOS;
  private searchTimer: any;
  form: any = this.emptyForm();
  emptyForm() { return { doador_id:'', tipo_sanguineo:'' as any, data_doacao: new Date().toISOString().split('T')[0], volume_ml:450, status:'coletada' as StatusDoacao, observacoes:'' }; }
  ngOnInit() { this.load(); this.loadDoadores(); }
  loadDoadores() { this.doaSvc.getAll({ limit: 100, status: 'ativo' }).subscribe({ next: r => this.doadores_lista.set(r.data) }); }
  load() {
    this.loading.set(true);
    this.svc.getAll({ page: this.page(), limit: 20 }).subscribe({
      next: r => { this.doacoes.set(r.data); this.total.set(r.total); this.pages.set(r.pages); this.loading.set(false); },
      error: () => { this.toast.error('Erro ao carregar doações.'); this.loading.set(false); }
    });
  }
  ...
  openModal(d?: Doacao) { this.editId.set(d?.id ?? null); this.form = d ? { ...d } : this.emptyForm(); this.showModal.set(true); }
  closeModal() { this.showModal.set(false); }
  save() {
    if (!this.form.doador_id) { this.toast.error('Selecione um doador.'); return; }
    this.saving.set(true);
    const obs = this.editId() ? this.svc.update(this.editId()!, this.form) : this.svc.create(this.form);
    obs.subscribe({ next: () => { this.toast.success(this.editId() ? 'Doação atualizada!' : 'Doação registrada!'); this.saving.set(false); this.closeModal(); this.load(); }, error: () => this.saving.set(false) });
  }
  delete(id: number) {
    if (!confirm('Excluir esta doação?')) return;
    this.svc.delete(id).subscribe({ next: () => { this.toast.success('Doação excluída.'); this.load(); } });
  }
  tipoCss(t: TipoSanguineo) { return t.startsWith('AB') ? 'badge-tipo tipo-ab' : t.includes('+') ? 'badge-tipo tipo-pos' : 'badge-tipo tipo-neg'; }
  sLabel(s: string) { return { coletada:'Coletada', em_analise:'Em Análise', aprovada:'Aprovada', rejeitada:'Rejeitada', utilizada:'Utilizada' }[s] ?? s; }
}
