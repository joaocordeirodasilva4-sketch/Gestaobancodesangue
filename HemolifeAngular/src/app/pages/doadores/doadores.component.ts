...
import { Doador, DoadorCreate, TipoSanguineo, StatusDoador } from '../../core/models';
...
@Component({
  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div><h1>Doadores</h1><p>{{ total() }} doadores cadastrados</p></div>
        <button class="btn-primary" (click)="openModal()">+ Novo Doador</button>
      </div>
      ...
      <div class="search-bar">
        ...
        <input [(ngModel)]="search" placeholder="Buscar por nome, tipo sanguíneo ou CPF..." (input)="onSearch()" />
      </div>

      <div class="table-wrap">
        @if (loading()) {
          <div class="table-loading">@for(i of [1,2,3,4,5]; track i){<div class="skel"></div>}</div>
        } @else {
          <table>
            <thead><tr><th>Nome</th><th>Tipo</th><th>Contato</th><th>Gênero</th><th>Status</th><th>Ações</th></tr></thead>
            <tbody>
              @for (d of doadores(); track d.id) {
                <tr>
                  <td><p class="nome">{{ d.nome_completo }}</p><p class="sub">{{ d.cpf ?? '—' }}</p></td>
                  <td><span class="badge-tipo" [class]="tipoCss(d.tipo_sanguineo)">{{ d.tipo_sanguineo }}</span></td>
                  <td><p>{{ d.telefone }}</p>@if(d.email){<p class="sub">{{ d.email }}</p>}</td>
                  <td>{{ gLabel(d.genero) }}</td>
      ...
      <div class="overlay" (click)="closeModal()">
        <div class="modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h2>{{ editId() ? 'Editar Doador' : 'Novo Doador' }}</h2>
          ...
          <div class="form-row">
            <div class="form-group"><label>Data de Nascimento</label><input [(ngModel)]="form.data_nascimento" type="date"/></div>
          ...
          <div class="form-group"><label>Status</label>
            <select [(ngModel)]="form.status"><option value="ativo">Ativo</option><option value="inativo">Inativo</option><option value="inapto_temporario">Inapto Temporário</option></select>
          </div>
  ...
  styles: [`
    ...
    .table-loading{padding:16px;display:flex;flex-direction:column;gap:10px}.skel{height:52px;background:linear-gradient(90deg,#f3f4f6 25%,#e5e7eb 50%,#f3f4f6 75%);background-size:200%;animation:shimmer 1.4s infinite;border-radius:6px}
    ...
    .badge-tipo{display:inline-flex;align-items:center;justify-content:center;min-width:36px;height:22px;border-radius:6px;font-size:11px;font-weight:700;color:#fff;padding:0 6px}
    .tipo-pos{background:#16a34a}.tipo-neg{background:#c0392b}.tipo-ab{background:#7c3aed}
    .badge-status{display:inline-block;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600}.st-ativo{background:#dcfce7;color:#16a34a}.st-inativo{background:#f3f4f6;color:#6b7280}.st-inapto_temporario{background:#fef3c7;color:#d97706}
...
export class DoadoresComponent implements OnInit {
  private svc   = inject(DoadoresService);
  private toast = inject(ToastService);
  doadores = signal<Doador[]>([]); total = signal(0); page = signal(1); pages = signal(1);
  loading = signal(true); saving = signal(false); showModal = signal(false); editId = signal<number|null>(null);
  search = ''; tipos = TIPOS;
  private searchTimer: any;
  form: any = this.emptyForm();
  emptyForm() { return { nome_completo:'',cpf:'',tipo_sanguineo:'' as any,telefone:'',email:'',data_nascimento:'',genero:'' as any,endereco:'',status:'ativo' as StatusDoador,observacoes:'' }; }
  ngOnInit() { this.load(); }
  load() {
    this.loading.set(true);
    this.svc.getAll({ page: this.page(), limit: 20, search: this.search || undefined }).subscribe({
      next: r => { this.doadores.set(r.data); this.total.set(r.total); this.pages.set(r.pages); this.loading.set(false); },
      error: () => { this.toast.error('Erro ao carregar doadores.'); this.loading.set(false); }
    });
  }
  ...
  openModal(d?: Doador) { this.editId.set(d?.id ?? null); this.form = d ? { ...d } : this.emptyForm(); this.showModal.set(true); }
  closeModal() { this.showModal.set(false); }
  save() {
    if (!this.form.nome_completo || !this.form.tipo_sanguineo || !this.form.telefone || !this.form.genero) { this.toast.error('Preencha os campos obrigatórios.'); return; }
    this.saving.set(true);
    const obs = this.editId() ? this.svc.update(this.editId()!, this.form) : this.svc.create(this.form);
    obs.subscribe({ next: () => { this.toast.success(this.editId() ? 'Doador atualizado!' : 'Doador cadastrado!'); this.saving.set(false); this.closeModal(); this.load(); }, error: () => this.saving.set(false) });
  }
  delete(id: number, nome: string) {
    if (!confirm(`Excluir "${nome}"?`)) return;
    this.svc.delete(id).subscribe({ next: () => { this.toast.success('Doador excluído.'); this.load(); } });
  }
  tipoCss(t: TipoSanguineo) { return t.startsWith('AB') ? 'badge-tipo tipo-ab' : t.includes('+') ? 'badge-tipo tipo-pos' : 'badge-tipo tipo-neg'; }
  gLabel(g: string) { return { masculino:'Masculino', feminino:'Feminino', outro:'Outro' }[g] ?? g; }
  sLabel(s: string) { return { ativo:'Ativo', inativo:'Inativo', inapto_temporario:'Inapto Temporário' }[s] ?? s; }
}
